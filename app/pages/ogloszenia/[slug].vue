...
function currentUrl(): string {
    ...
    // Kanoniczny, przyjazny adres - trafia do <link rel="canonical">, kodu QR i udostępniania.
    return `${config.public.siteUrl}${advert.value ? advertPath(advert.value) : `/ogloszenia/${id}`}`
}
...
async function downloadPDF() {
    ...
    try {
        ...
        doc.text(`Wygenerowano: ${new Date().toLocaleDateString('pl-PL')} | carizo.eu${advertPath(a)}`, 14, 285)
...
// Kanoniczny adres. Ta sama oferta jest osiągalna pod każdym adresem kończącym się
// jej ID (stary /advert/123, ręcznie skrócony, albo z nieaktualnym tytułem po edycji).
// Dla użytkownika to wygoda, ale dla wyszukiwarki to duplikaty tej samej treści -
